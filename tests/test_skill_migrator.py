...
""",
    ...
    def test_skill_documentation_is_utf8_and_has_no_removed_commands(self) -> None:
        ...
        playbook = (REPO_ROOT / "references" / "migration-playbook.md").read_text(encoding="utf-8")

        self.assertIn("Claude Code", text)
        self.assertIn("Hermes", text)
        self.assertIn("让 OpenClaw 学会把 Claude Code 和 Hermes skill 转成 OpenClaw skill。", text)
        self.assertIn("## Quick Decision Tree", text)
        self.assertIn("## Review Checkpoints", text)
        self.assertIn("## Minimal Examples", text)
        self.assertIn("v4.5 - 保持真实边界，补回决策树、检查点与最小示例。", text)
        ...
        self.assertIn("v4.5 - Keep boundaries honest, add back the decision tree, checkpoints, and minimal examples.", readme)
        self.assertIn("v4.5 - 保持真实边界，补回决策树、检查点与最小示例。", readme)
        self.assertIn("migration-playbook.md", readme)
        ...
        self.assertIn("## Review Checkpoints", playbook)
        self.assertIn("## Detailed Scenarios", playbook)
        self.assertIn("## Detailed Diagnostics", playbook)
        self.assertIn("## Repair Choices", playbook)
        self.assertNotIn("auto-fix", playbook)
        self.assertNotIn("自动修复", playbook)
